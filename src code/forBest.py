...
def training(model,training_loader,epoch=300):
    ...
    print(f'max test score= {max(test_record)} at epoch {epoch_index}\n')
...
if __name__=="__main__":
    ...
    modelSaveDir=f'./best_{current_time}'
    ...
    #eeg_model=torch.load("./07_17-56-18/EEGNet_LeakyReLU").to(device)
    eeg_model,_,eeg_train,eeg_test,best_score=training(eeg_model,training_loader)
        
    torch.save(eeg_model,f'{modelSaveDir}/EEGNet_{actFunc}')
    
    X=[i+1 for i in range(len(eeg_train))]
    plt.plot(X,eeg_train,'-',label=f"{actFunc}_train")
    plt.plot(X,eeg_test,'-',label=f"{actFunc}_test")
    f = open("Best.txt", "a")
    f.write(f'\n{modelSaveDir}   epoch:{len(eeg_train)} batch:{batch_size}\n')
    f.write(f'EEG_{actFunc:15s} {eeg_test[-1]:.6f} best: {best_score}\n')
    
    print(f'EEG_{actFunc}: {eeg_test[-1]}\n')
    ...
    for actFunc in activate_func_list:
        ...
        print(f'deepConv_{actFunc}: {deepConv_test[-1]}\n')
